...
import { ArrowUpDown, Image as ImageIcon, Play, Video } from "lucide-react";
...
export function ListView({ events, onEventClick }: ListViewProps) {
	...
	const sortedEvents = useMemo(() => {
		return [...events].sort((a, b) => {
			...
			switch (sortField) {
				case "title":
					comparison = (a.description || a.title).localeCompare(
			...
			<Table>
				...
				<TableBody>
					{sortedEvents.map((event) => {
						const badgeStyle =
							statusBadgeStyles[event.status] || statusBadgeStyles.draft;
