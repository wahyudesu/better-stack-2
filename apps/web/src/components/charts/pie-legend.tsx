...
function renderChildren(
  ...
  if (Array.isArray(children)) {
    return children.map((child, idx) => (
      <React.Fragment key={context.childIndex !== undefined ? `${context.index}-${idx}` : idx}>
...
function renderChild(
  ...
  if (child && typeof child === "object" && "type" in child) {
    ...
    const props = child.props as { children?: ReactNode; className?: string; showValue?: boolean; valueSuffix?: string };
...
export function LegendItemComponent({
  ...
  const isFaded = hoveredIndex !== null && hoveredIndex !== undefined && !isHovered;
