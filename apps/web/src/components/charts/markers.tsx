...
export function ChartMarkers({ items, size = 20, showLines = true }: ChartMarkersProps) {
  ...
  const handleMarkerClick = (marker: ChartMarkerItem & { originalIndex: number }, e: React.MouseEvent) => {
    e.stopPropagation();
      ...
      {markers.map((marker) => {
        ...
        return (
          <g
            ...
            onMouseEnter={() => hasInteraction && setHoveredIndex(marker.originalIndex)}
