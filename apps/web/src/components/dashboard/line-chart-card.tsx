...
export function LineChartCard({ chartData, markers, formatMetricValue, keyProp }: LineChartCardProps) {
              ...
              {metricOptions
                ...
                .map((option) => (
                  <DropdownMenuItem
                    key={option.value}
                    onClick={() =>
                      setSecondaryMetric(secondaryMetric === option.value ? null : option.value)
                    }
                  >
                    <span className="flex-1">{option.label}</span>
