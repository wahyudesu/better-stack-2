...
const baseStyles = cva(
	"rounded-lg inline-flex shrink-0 items-center justify-center cursor-pointer select-none active:translate-y-1 active:[box-shadow:0_0px_0_0_var(--depth-shadow-blue-active),0_0px_0_0_var(--depth-shadow-blue-active)_/_0.25] active:border-b-[0px] transition-all duration-150 text-white font-medium text-sm outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
	{
		variants: {
			...
			variant: {
				outline: "bg-background hover:bg-muted text-foreground border-border border-1 dark:border-input dark:bg-background dark:hover:bg-input/50 [box-shadow:0_3px_0_0_var(--border)]",
				destructive: "bg-destructive/10 text-destructive hover:bg-destructive/20 border-1 border-destructive dark:bg-destructive/20 dark:hover:bg-destructive/30 [box-shadow:0_3px_0_0_var(--destructive)]",
...
interface DepthButtonProps extends React.ComponentProps<"button">, VariantProps<typeof baseStyles> {}
...
function DepthButtonGroup({ className, children, ...props }: DepthButtonGroupProps) {
	return (
		<div
...
const groupButtonStyles = cva(
	baseStyles,
	{
		...
		defaultVariants: {
			...baseStyles.defaultVariants,
			position: "single",
		},
	}
)

interface GroupedDepthButtonProps extends React.ComponentProps<"button">, VariantProps<typeof groupButtonStyles> {
