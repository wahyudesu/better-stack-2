...
const tools = [
	{
		...
		icon: FileText,
		description: "Generate a draft script",
	},
	{
		...
		<div className="space-y-4">
			<div className="grid gap-4 sm:grid-cols-3">
				<div className="space-y-2">
					...
					<Select
						value={platform}
						onValueChange={(v) => setPlatform(v ?? "instagram")}
					>
						<SelectTrigger>
							<SelectValue />
						</SelectTrigger>
			...
			<Button
				onClick={() =>
					topic.trim() && setOutput(generateScript(platform, topic, tone))
				}
				disabled={!topic.trim()}
				size="sm"
				className="gap-1.5"
...
function BrandingTool() {
	...
	return (
		<div className="space-y-4">
			...
			<Button
				onClick={() => name.trim() && setOutput(generateBranding(name, niche))}
				disabled={!name.trim()}
				size="sm"
				className="gap-1.5"
			>
