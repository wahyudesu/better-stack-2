...
function gaussianNoise(mean: number, stdDev: number, rand: () => number): number {
  const u1 = rand();
  const u2 = rand();
...
function generateData(
  ...
  const chartData: ChartDataPoint[] = Array.from({ length: days }, (_, i) => {
    ...
    const views = Math.max(
      engagements * platformMult.impressions * typeMult.impressions * (2 + random() * 2),
    );
    const comments = Math.max(
      10,
      Math.floor(engagements * platformMult.replies * typeMult.replies * (0.1 + random() * 0.1)),
    );
    const impression = Math.max(
      engagements * platformMult.impressions * typeMult.impressions * (3 + random() * 2),
    );
    const share = Math.max(
      5,
      Math.floor(engagements * platformMult.shares * typeMult.shares * (0.05 + random() * 0.1)),
    );
  ...
  const stats = [
    {
      ...
      change: formatChange(
        calcPercentChange(totalImpressions, totalImpressions * (0.82 + random() * 0.15)),
      ),
    ...
    },
    {
      ...
      change: formatChange(calcPercentChange(totalLikes, totalLikes * (0.80 + random() * 0.2))),
      metricKey: "followers",
    },
    {
      label: "Replies",
      ...
      change: formatChange(
        calcPercentChange(totalComments, totalComments * (0.65 + random() * 0.25)),
      ),
      metricKey: "comments",
    },
    {
      ...
      change: formatChange(calcPercentChange(totalShares, totalShares * (0.82 + random() * 0.18))),
      metricKey: "share",
    },
    {
      label: "Saves",
      value: formatMetricValue(totalSaves),
      change: formatChange(calcPercentChange(totalSaves, totalSaves * (0.75 + random() * 0.2))),
      metricKey: "saves",
    },
  ];
