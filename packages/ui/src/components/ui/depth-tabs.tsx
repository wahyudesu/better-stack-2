...
const depthTabStyles = cva(
  [
    "relative inline-flex shrink-0 items-center justify-center cursor-pointer select-none",
    ...
    "[box-shadow:0_3px_0_0_var(--border)] border border-input",
    // Depth effect - active/selected state (pressed in)
    "data-[selected]:[box-shadow:0_0px_0_0_var(--border)] data-[selected]:translate-y-[3px] data-[selected]:border-b-[0px]",
    // Hover - subtle lift
    "hover:[box-shadow:0_4px_0_0_var(--border)] hover:translate-y-[-1px]",
  ],
  {
    variants: {
      size: {
...
function DepthTabsTrigger({
  ...
  return (
    <TabsPrimitive.Tab
      ...
      className={cn(
        depthTabStyles({ size }),
        "rounded-md bg-background hover:bg-accent hover:text-accent-foreground data-[selected]:bg-primary data-[selected]:text-primary-foreground data-[selected]:border-primary",
        className
      )}
      {...props}
    />
